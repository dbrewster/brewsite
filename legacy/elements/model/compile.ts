...
import {OPAQUE_OPACITY} from '../../model/bodyPartOverrides';
 ...
 * This was in animationTiming.ts but belongs in compile-time logic.
 * Moving here fixes the compiler→runtime import violation.
 */
...
// Transition spec helpers (copied from modelTransitions.ts)
...
export const poseGroupTransition = (from?: RobotPoseGroup, to?: RobotPoseGroup, t?: number) => {
...
// Model transition spec (renamed from modelTransitions.ts)
...
// Playback transition spec helpers (from playbackTransitions.ts)

const blendPoseGroups = (
  from?: Partial<Record<string, RobotPoseGroup>>,
  to?: Partial<Record<string, RobotPoseGroup>>,
  t?: number,
) => {
  if (!from && !to) return undefined;
  const result: Partial<Record<string, RobotPoseGroup>> = {};
  const keys = new Set<string>([...Object.keys(from ?? {}), ...Object.keys(to ?? {})]);
  for (const key of keys) {
    const prev = from?.[key];
    const next = to?.[key];
    if (!prev && !next) continue;
    result[key] = (poseGroupTransition(prev, next, t) ?? prev ?? next) as RobotPoseGroup;
  }
  return Object.keys(result).length > 0 ? result : undefined;
};

const blendCommands = (
  from?: RobotMotionCommand[],
  to?: RobotMotionCommand[],
  tExit?: number,
  tEnter?: number,
  tFull?: number,
) => {
  if (!from && !to) return [];
  const result: RobotMotionCommand[] = [];
  const byId = new Map<string, RobotMotionCommand>();
...
const blendMotionScenes = (
  ...
  if (!from && !to) return [];
  const result: RobotMotionScene[] = [];
  const byId = new Map<string, RobotMotionScene>();
...
// Playback transition spec (renamed from playbackTransitions.ts)
...
// compileAnimation function (from sceneTrackCompiler.ts)
...
export const compileAnimation = (options: {
  sceneId: string;
  ...
  const clip = clipName ? options.availableClips.find((item) => item.name === clipName) : undefined;

  const animationSettings = options.playback.animation;
  const expectsLoadedClip = !animationSettings.gltfUrl && !animationSettings.fbxUrl;
  if (
    animationState.hasAnimationRequest &&
    clipName &&
    !clip &&
    options.availableClips.length > 0 &&
    expectsLoadedClip
  ) {
    const key = `${options.sceneId}:${clipName}`;
    if (!options.warnOnce.has(key)) {
      options.warnOnce.add(key);
      console.warn('[RobotSceneCompiler]', 'missing.animation.clip', {
        sceneId: options.sceneId,
        clipName,
        available: options.availableClips.map((item) => item.name),
      });
    }
  }
  ...
  const range = resolveClipRangeSeconds(options.playback.animation, clip.duration);
  return {
    enabled: true,
    clipName,
