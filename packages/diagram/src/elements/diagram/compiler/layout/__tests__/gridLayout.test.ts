...
describe('resolveGridLayout', () => {
  it('places single node with y=0 and z=0', () => {
    ...
    const result = resolveGridLayout(nodes, [], grid({ columns: 1 }), [0.15, 0.08]);
  ...
  it('places two nodes in a single row with spacing', () => {
    ...
    const result = resolveGridLayout(nodes, [], grid({ columns: 2, spacing }), [0.15, 0.08]);
    const a = result.get('a')!;
    const b = result.get('b')!;
    // a is left, b is right; b.x > a.x
    expect(b[0]).toBeGreaterThan(a[0]);
    // gap between centers = w + spacing = 0.15 + 2 = 2.15
    expect(b[0] - a[0]).toBeCloseTo(0.15 + 2);
  });

  it('wraps to a second row with a 1-column layout', () => {
    const nodes = [makeNode('a'), makeNode('b'), makeNode('c')];
    const result = resolveGridLayout(nodes, [], grid({ columns: 1, spacing: [2, 2] }), [0.15, 0.08]);
    const a = result.get('a')!;
    const b = result.get('b')!;
  ...
  it('places auto-columns (4) when columns=auto', () => {
    ...
    const result = resolveGridLayout(nodes, [], grid({ columns: 'auto' }), [0.15, 0.08]);
  ...
  it('preserves explicit positions', () => {
    ...
    const result = resolveGridLayout(nodes, [], grid(), [0.15, 0.08]);
  ...
  it('respects center alignment', () => {
    ...
    const nodes = [makeNode('a'), makeNode('b'), makeNode('c')];
    const result = resolveGridLayout(nodes, [], grid({ columns: 2, alignment: 'center' }), [0.15, 0.08]);
