...
const allClipNames = Array.from(
    new Set(Object.values(animationRegistry).map((entry) => entry.clipName).filter(Boolean)),
).sort();
const clipTypeLines = modelDefs.map((entry) => {
    const typeName = `${toPascalCase(entry.type)}ClipName`;
    const clipUnion = allClipNames.length > 0 ? allClipNames.map((name) => JSON.stringify(name)).join(' | ') : 'never';
    return `export type ${typeName} = ${clipUnion};`;
}).join('\n');
