  ...
  const canvasDefault = compileCanvas(
    ...
    [
      compileDiagram({
        ...
        pivot: 'center',
        nodes: [
          { id: 'browser', label: 'Web Browser', position: [-6, 6, 0], icon: 'ui:user' },
          { id: 'cdn', label: 'CloudFront CDN', position: [0, 2, 0], icon: 'aws:cloudfront' },
          { id: 'alb', label: 'Load Balancer', position: [0, -1, 0], icon: 'aws:alb' },
          { id: 'api', label: 'API Gateway', position: [0, -4, 0], icon: 'aws:api-gateway' },
          { id: 'ecs', label: 'ECS Cluster', position: [-5, -8, 0], icon: 'aws:ecs' },
          { id: 'lambda', label: 'Lambda', position: [5, -8, 0], icon: 'aws:lambda' },
          { id: 'rds', label: 'RDS PostgreSQL', position: [-5, -13, 0], icon: 'aws:rds' },
          { id: 'cache', label: 'ElastiCache', position: [0, -13, 0], icon: 'aws:elasticache' },
          { id: 's3', label: 'S3 Assets', position: [5, -13, 0], icon: 'aws:s3' },
        ],
        edges: [
