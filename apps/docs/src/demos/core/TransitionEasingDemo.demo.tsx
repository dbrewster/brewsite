...
<Scene key="end" transition={{ easing: 'easeInOut' }}>
...
export default function TransitionEasingDemo(): JSX.Element {
  const [selectedEasing, setSelectedEasing] = useState<EasingName>('easeInOut');
  ...
  return (
    <div>
      ...
      <DemoScene key={selectedEasing} sceneCount={2}>
        ...
        <Scene key="end" transition={{ easing: selectedEasing }}>
          <Camera mode="world" position={[3, 3, 6]} target={[0, 0, 0]} />
          <Lighting>
            <Ambient color="#8855ff" intensity={0.6} />
          </Lighting>
