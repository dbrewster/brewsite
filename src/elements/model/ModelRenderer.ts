...
export class ModelRenderer {
  ...
  async loadGlb(glbUrl: string, options?: LoadOptions): Promise<void> {
    ...
    const gltf = await new Promise<THREE.Group & { animations?: THREE.AnimationClip[] }>((resolve, reject) => {
      loader.load(
        glbUrl,
        (loaded) => resolve(Object.assign(loaded.scene, { animations: loaded.animations })),
        undefined,
        (error) => reject(error),
      );
    });
    ...
    this.ingestModel(gltf, gltf.animations ?? []);
  ...
  apply(
    ...
    this.applyBodyPartOverrides(state);

    // Apply contained model attachments
    this.applyModelParts(state.model.parts ?? {});

    // Apply animations
    if (animation?.enabled && animation.clipName) {
      this.applyAnimation(state, animation, ctx);
    } else {
  ...
  private async loadAnimationClips(loader: GLTFLoader, animations: AnimationEntry[]): Promise<void> {
    ...
    const loads = animations.map((entry) =>
      new Promise<{ entry: AnimationEntry; clips: THREE.AnimationClip[] }>((resolve, reject) => {
        loader.load(
          entry.glb,
          (gltf) => resolve({ entry, clips: gltf.animations ?? [] }),
          undefined,
          (error) => reject(error),
        );
      }),
  ...
  private applyBodyPartOverrides(state: SceneModelInstanceState): void {
    if (!this.model) return;
  ...
  private applyModelParts(parts: Record<string, ModelPartSpec>): void {
    ...
    for (const part of Object.values(parts)) {
      ...
      const anchorNode = this.nodeByName.get(anchorName);
      ...
      this.applyContainedOverrides(instance.group, part);
    }
  }
  ...
  private getOrCreateContainedInstance(part: ModelPartSpec): ContainedInstance | null {
    ...
    const instance = { modelId: part.modelId ?? '', group: clone };
    this.attachedParts.set(part.id, instance);
    return instance;
  }

  private applyContainedOverrides(group: THREE.Group, part: ModelPartSpec): void {
    group.traverse((obj) => {
      ...
      if (typeof part.opacity === 'number') {
        mesh.visible = part.opacity > 0;
      }
    });
    ...
    for (const [id, spec] of Object.entries(subparts) as Array<[string, ModelSubpartSpec]>) {
      ...
      if ((node as THREE.Mesh).isMesh || node.type === 'Mesh') {
        ...
        mesh.visible = spec.enabled !== false && (spec.opacity ?? 1) > 0;
      }
    }
  }
  ...
  private applyAnimation(
    ...
    const clip = this.animationClips.get(animation.clipName ?? '');
    if (!clip) return;

    if (this.activeClipName !== clip.name) {
      this.clearActiveAnimation();
      ...
      this.animationTimeSeconds = animation.range?.startSeconds ?? 0;
    }

    const deltaSeconds = ctx?.deltaSeconds ?? 0;
    const range = animation.range;
    const repeat = state.playback.animation.clipRepeat !== false;
    const span = range?.span ?? clip.duration;
    const start = range?.startSeconds ?? 0;
    const end = range?.endSeconds ?? clip.duration;

    let next = this.animationTimeSeconds + deltaSeconds;
    if (repeat) {
      const local = (next - start) % span;
      next = start + (local < 0 ? local + span : local);
    } else {
      next = Math.max(start, Math.min(end, next));
    }
    this.animationTimeSeconds = next;
    this.mixer.setTime(next);
  ...
  private clearActiveAnimation(): void {
    if (!this.mixer || !this.activeClipName) return;
    const clip = this.animationClips.get(this.activeClipName);
    if (clip) {
