...
export class MockAnimationPlayer implements AnimationPlayer {
  ...
  load(tracks: AnimationTrack[]): void {
    ...
    this.tracks = tracks.map((track) => {
      targetNames.add(track.targetName);
      return { ...track, keyframes: track.keyframes.map((frame) => ({ ...frame })) };
    });
    const renamed: AnimationTrack[] = [];
    for (const track of this.tracks) {
      const mapped = filterAndRenameTrack(`${track.targetName}.${track.property}`, targetNames);
      if (!mapped.allowed) continue;
      const dot = mapped.name.indexOf('.');
      const targetName = dot > 0 ? mapped.name.slice(0, dot) : track.targetName;
      renamed.push({ ...track, targetName });
    }
    this.tracks = renamed;
  }
