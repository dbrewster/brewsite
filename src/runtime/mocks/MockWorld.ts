...
/**
 * Mock implementation of World for testing.
 * Stub - implemented in Phase 6
 */
export class MockWorld implements World {
  readonly nodesByName: Map<string, Node> = new Map();
  readonly root: Node;
  ...
  updateWorldMatrix(): void {
    const updateNode = (node: Node, parentMatrix?: number[]) => {
      ...
      const worldMatrix = parentMatrix
        ? multiplyMatrices(parentMatrix as any, localMatrix)
        : localMatrix;
      const decomposed = decomposeMatrix(worldMatrix as any);
      (node as any).worldPosition = copyVec3(decomposed.position);
      (node as any).worldRotation = copyVec3(decomposed.rotation);
      (node as any).worldScale = copyVec3(decomposed.scale);
      (node as any).matrixWorld = worldMatrix;
