import type { BodyPartOverrideMap } from '../types';
...
export class MockModel implements Model {
  ...
  applyMaterialOverrides(
    ...
    _options?: { metalness?: number; roughness?: number; opacity?: number },
  ): void {
    for (const [targetName, value] of Object.entries(overrides)) {
      ...
      if (existing) {
        ...
        (node as any).components = [...(node.components ?? []), { type: 'materialOverride', props }];
      }
    }
  }
    ...
    objects.forEach((obj) => {
      ...
      if (obj.type === 'model') {
        ...
        if (existing) {
          ...
          (anchorChild as any).components = [...(anchorChild.components ?? []), { type: obj.componentType, props: nextProps }];
        }
