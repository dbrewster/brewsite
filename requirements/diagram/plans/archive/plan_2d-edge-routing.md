...
**What's eliminated:**
- Underpass logic (Z-dip around obstacles) — fully removed (see Underpass Removal section)
...
### `pathBuilder.ts` — Path Building + Profile Shaping + Z Assignment

**Replaces:** `flowPathBuilder.ts` (349 LOC) + `curveKernel.ts` (110 LOC) + `edgeRoutingProfiles.ts` (536 LOC) — total 995 LOC.

**Target:** ~350 LOC.

All four profile functions live in this single module. No separate `pathProfiles.ts` file. The profile is a branch in the orchestrator that calls one of four `pathBuilder` functions. All four produce `PathCommand2D[]`. Corner smoothing is integral to path building, not a separate step.
...
export function routeEdges(
  ...
  for (const edge of edges) {
    ...
    if (edge.fromId === edge.toId) { results.set(edge.id, EMPTY_ROUTE); continue; }
...
**Inline helpers (not separate modules):**
...
function mirrorRouteY(result: EdgeRouteResult): EdgeRouteResult {
  // Negate y component of every Vec3 in path commands and control points.
  // ~10 lines of point negation across line/cubic command variants.
}
```
...
The old pipeline supported `allowUnderpass` — edges could dip in Z to pass under obstacles. This is a 3D concept that doesn't fit the 2D routing model. All underpass-related fields, props, and theme values are **fully deleted** in this plan. This is a **major version** bump.

**DSL props — DELETE:**
- `allowUnderpass` on `DiagramEdgeDSL` (types.ts:1383) — delete the prop entirely.
- `allowUnderpass` on `DiagramEdgeState` (types.ts:1046) — delete the field entirely.

**Output fields — DELETE:**
- `usedUnderpass` on `DiagramEdgePathState` (types.ts:938) — delete the field entirely. Confirmed: this field is purely informational (used in debug overlay only; does not gate render behavior). All test fixtures that set `usedUnderpass: false` must also be updated to remove the field.

**Theme type definitions — DELETE:**
- `flowUnderpassDepth` on `DiagramEdgeTheme` (types.ts:218) — delete.
- `flowUnderpassClearance` on `DiagramEdgeTheme` (types.ts:220) — delete.
- `flowUnderpassPenalty` on `DiagramEdgeTheme` (types.ts:226) — delete.

**Theme preset values — DELETE from all 6 presets:**
- `packages/diagram/src/elements/diagram/themes/darkGlass.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/diagram/src/elements/diagram/themes/midnight.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/diagram/src/elements/diagram/themes/enterprise.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/diagram/src/elements/diagram/themes/neonCyber.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/diagram/src/elements/diagram/themes/lightMinimal.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/diagram/src/elements/diagram/themes/lightCanvas.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.

**Theme presets in `packages/themes/src/` — DELETE from all 6 presets:**
- `packages/themes/src/presets/diagram/darkGlass.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/themes/src/presets/diagram/midnight.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/themes/src/presets/diagram/enterprise.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/themes/src/presets/diagram/neonCyber.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/themes/src/presets/diagram/lightMinimal.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.
- `packages/themes/src/presets/diagram/lightCanvas.ts` — delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`.

**Internal routing types — no underpass fields exist:**
- New `FlowConfig` omits underpass fields entirely — the orchestrator does not pass them through.
- `OrthogonalRouteResult` has no `usedUnderpass` field.

**Old source files with underpass references (all deleted in Phase 9):**
The following files in the old pipeline reference underpass fields and will be deleted entirely:
- `compiler/edgeCandidateScorer.ts` — `UNDERPASS_PENALTY` constant and `usedUnderpass` scoring.
- `compiler/flowPathBuilder.ts` — `usedUnderpass` on `OrthogonalRouteResult` and passthrough.
- `compiler/flowVisibilityGraph.ts` — `buildUnderpassRoute()`, `usedUnderpass`, `underpassPenalty`, `underpassDepth`, `underpassClearance`, `allowUnderpass`.
- `compiler/flowRouter.ts` — `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`, `allowUnderpass`, `usedUnderpass`.
- `compiler/edgeRoutingProfiles.ts` — `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`, `allowUnderpass`, `usedUnderpass`.
- `compiler/edgeRouter.ts` — `allowUnderpass`, `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`, `usedUnderpass`.
- `compiler/routingTypes.ts` — `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`, `allowUnderpass`, `usedUnderpass`.
- `compiler/defaultsCompiler.ts` — `allowUnderpass` field and default value.
- `compiler/nodeCompiler.ts` — `allowUnderpass` DSL-to-state mapping.
- `compiler/transitionHelpers.ts` — `allowUnderpass` passthrough in `rerouteLiveEdges`.
- `compile.ts` — `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty`, `usedUnderpass` in default path state.
- `dsl.tsx` — `allowUnderpass` DSL prop definition.

---

## Non-Routing Code Assessment

The 3D face model is **properly confined to the routing pipeline**. The rest of the diagram package is clean except for `transitionHelpers.ts` and `compile.ts` which have direct routing imports.
...
|---|---|---|
| `compiler/transitionHelpers.ts` | **DIRECT DEPENDENCY** | Imports `routeEdges`/`routeEdgesYDown` from `./edgeRouter` and `optimizeSharedFlowTrunks` from `./edgeRenderOptimizer`. See detailed instructions below. |
| `compile.ts` | **CALL SITE + BREAKING** | One `routeEdgesYDown()` call (line 267) → update to new `routeEdges()`. Re-export on line 598 → update import path. Remove `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty` from the routing config construction (lines 281–285). Remove `usedUnderpass: false` from the default path state (line 314). |
| `types.ts` | **BREAKING** | Delete `allowUnderpass` from `DiagramEdgeDSL` and `DiagramEdgeState`. Delete `usedUnderpass` from `DiagramEdgePathState`. Delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty` from `DiagramEdgeTheme`. Update `DiagramEdgePort` type alias if `FaceId` is renamed. |
| `dsl.tsx` | **BREAKING** | Delete `allowUnderpass` prop from `DiagramEdgeDSL` component interface (line 212). |
| Theme presets (6 files in `themes/`) | **BREAKING** | Delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty` from `darkGlass.ts`, `midnight.ts`, `enterprise.ts`, `neonCyber.ts`, `lightMinimal.ts`, `lightCanvas.ts`. |
| Theme presets (6 files in `packages/themes/src/presets/diagram/`) | **BREAKING** | Same deletions as above in the external `@brewsite/themes` package. |
| `compiler/defaultsCompiler.ts` | **BREAKING** | Delete `allowUnderpass` field from `CompiledEdgeDefaults` type (line 54) and its default value `allowUnderpass: true` (line 135). |
| `compiler/nodeCompiler.ts` | **BREAKING** | Delete `allowUnderpass` DSL-to-state mapping (line 171). |
| `compiler/transitionHelpers.ts` | **BREAKING** | Remove `allowUnderpass` passthrough in edge state construction (lines 126, 142). |

#### `transitionHelpers.ts` Update Instructions
...
**`rerouteLiveEdges()` (lines 106–159):**

The function's **external signature stays the same** (`Vec3`/`NodeDimensions` maps). This is critical — callers in `functionalDiagramTransitionSpec` depend on this contract and must NOT change.

The **internal implementation** changes: convert the `Vec3`/`NodeDimensions` inputs to `NodeRect` maps, then call the new `routeEdges()`. This is ~10 lines of adapter code:
...
export function rerouteLiveEdges(
  ...
  const edgesForRouting = [...toEdges, ...fromEdges.filter((e) => !toEdgeIds.has(e.id))]
    .map((e) => ({
      id: e.id, fromId: e.fromId, toId: e.toId,
      profile: e.routing as 'flow' | 'curved' | 'straight' | 'organic',
      fromPort: e.fromPort as SideId | undefined,
      toPort: e.toPort as SideId | undefined,
      thickness: e.thickness,
    }));

  // Construct FlowConfig from defaults
  const config: FlowConfig = {
    turnRadius: DEFAULT_FLOW_CONFIG.turnRadius,
    faceStub: DEFAULT_FLOW_CONFIG.faceStub,
    obstaclePadding: DEFAULT_FLOW_CONFIG.obstaclePadding,
    turnPenalty: DEFAULT_FLOW_CONFIG.turnPenalty,
    punchthroughPenalty: DEFAULT_FLOW_CONFIG.punchthroughPenalty,
    bundleStrength: DEFAULT_FLOW_CONFIG.bundleStrength,
    organicVariation: DEFAULT_FLOW_CONFIG.organicVariation,
  };

  return routeEdges(edgesForRouting, nodeRects, groupIds, obstacleGroupIds, config);
}
```

**`blendDiagramEdges()` (line 193):** Only the import path changes for `optimizeSharedFlowTrunks`. The function signature and call site remain identical.

#### `compile.ts` Update Instructions
...
**Line ~267 (call site):** Update the `routeEdgesYDown` call to construct `NodeRect` maps and call the new `routeEdges()`. Same adapter pattern as `rerouteLiveEdges` above.
...
// NEW (re-export preserved, import path changed):
export { resolveLayout, computeBounds, routeEdges };
```

Note: `routeEdges` is exported from the element barrel `index.ts` (line 81) but is NOT re-exported from `packages/diagram/src/index.ts`. It is element-barrel-public, not npm-public. No external consumers exist. The function name is preserved but the signature changes.

---

## Type Compatibility Notes

**`EdgeRouteResult` (new) vs `EdgeRouteState` (old):** Same shape — `{ path: DiagramEdgePathState; controlPoints: ReadonlyArray<Vec3>; pathDebug?: DiagramEdgePathDebug }`. Renamed for clarity. All consumers updated in Phase 8.

**`FlowConfig` (new) vs `FlowRoutingConfig` (old):** New type drops `targetApproachBias`, `underpassDepth`, `underpassClearance`, `underpassPenalty`. The old type stays in the old code until Phase 9 deletion. `transitionHelpers.ts` currently passes `undefined` for the flow config in `routeEdgesYDown()` (line 155), which uses `DEFAULT_FLOW_ROUTING_CONFIG`. The new adapter constructs `FlowConfig` from the same default values.

**`DiagramEdgePathState.usedUnderpass`:** Deleted entirely. Was purely informational — used only in debug overlay, did not gate render behavior. All test fixtures that set this field must be updated to remove it.

---

## Migration Strategy: Build Alongside, Delete Last

The new routing code is built in a new `compiler/routing/` subdirectory. The old code stays untouched until all new code is validated. The build stays green throughout.

### Phase 1: Build `routing/routingTypes.ts`

Create `compiler/routing/`. Build types (`SideId`, `Vec2`, `Vec3`, `Rect2D`, `NodeRect`, `EdgeRoutingRequest`, `WaypointPath`, `EdgeRouteResult`, `FlowConfig`, `BundleHint`), Vec2/Vec3 math utilities, and `sideToApproach` helper.

**New tests:** `compiler/routing/__tests__/routingTypes.test.ts` — test Vec2 math edge cases (zero vectors, normalization of tiny vectors).

### Phase 2: Build `routing/sideSelect.ts`

Implement `nearestSide()`, `sideNormal()`, `sideCenter()`, `portAnchor()`, `selectSides()`, `inferBundleHints()`. This replaces the face enumeration + port assignment + guide generation pipeline.

**Degenerate cases to test:**
- Co-located nodes (same center): `nearestSide` returns `'top'`
- Equal magnitude dx/dy: prefer vertical (top/bottom)
- Locked ports override all algorithmic selection

**New tests:** `compiler/routing/__tests__/sideSelect.test.ts`

### Phase 3: Build `routing/obstacleModel.ts`
...
**Degenerate cases to test:**
- Source and dest in same group: group gets corridors for BOTH endpoints
- Nested groups (group inside group): each group evaluated independently for containment

**New tests:** `compiler/routing/__tests__/obstacleModel.test.ts`

### Phase 4: Build `routing/orthogonalRouter.ts`

Port the A* visibility graph algorithm from the existing `flowVisibilityGraph.ts`. Convert Vec3→Vec2, remove `planeZ`. All underpass logic (`buildUnderpassRoute`, `underpassPenalty`, `underpassDepth`, `underpassClearance`, `allowUnderpass`) is dropped entirely — not ported. Keep the A* search, obstacle intersection, turn penalty, corridor allowance.

**Degenerate cases to test:**
- Start === end: return single-waypoint path
- Start inside an obstacle: still produce a path (best-effort)
- No obstacles: return direct path (start → end)

**New tests:** `compiler/routing/__tests__/orthogonalRouter.test.ts`

### Phase 5: Build `routing/pathBuilder.ts`
...
**Specific test cases:**
- `buildFlowPath`: arc rounding at 90° turns, no sharp corners
- `buildCurvedPath`: single-waypoint (straight shot) vs multi-waypoint (S-curve)
- `buildStraightPath`: fallback to flow when obstacles cause turns
- `buildOrganicPath`: hash determinism (same edgeId + inputs → same outputs)
- `assignDepth`: smoothstep Z at t=0, 0.5, 1 for same-Z and different-Z nodes

**New tests:** `compiler/routing/__tests__/pathBuilder.test.ts`

### Phase 6: Build `routing/shapeSnap.ts` + `routing/trunkOptimizer.ts`
...
### Phase 7: Build `routing/edgeRouter.ts` Orchestrator

Build the thin orchestrator with `mirrorNodeRectsY` / `mirrorRouteY` inline helpers. Self-loop detection, missing endpoint warnings, profile dispatch.

**New tests:** `compiler/routing/__tests__/edgeRouter.test.ts` (unit tests for orchestration logic, self-loops, missing endpoints, profile dispatch)

### Phase 8: Wire Into `compile.ts` + Update Callers

Replace the old routing call in `compile.ts` with the new `routeEdges()`. Update `transitionHelpers.ts` with the adapter pattern (external signature preserved, internal implementation calls new router). Update re-export paths.

Delete all underpass fields from the public API surface:
- `types.ts`: Delete `allowUnderpass` from `DiagramEdgeDSL` (line 1383), `DiagramEdgeState` (line 1046). Delete `usedUnderpass` from `DiagramEdgePathState` (line 938). Delete `flowUnderpassDepth` (line 218), `flowUnderpassClearance` (line 220), `flowUnderpassPenalty` (line 226) from `DiagramEdgeTheme`.
- `dsl.tsx`: Delete `allowUnderpass` prop (line 212).
- `compile.ts`: Remove `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty` from routing config (lines 281–285). Remove `usedUnderpass: false` from default path state (line 314).
- `compiler/defaultsCompiler.ts`: Delete `allowUnderpass` field (line 54) and default value (line 135).
- `compiler/nodeCompiler.ts`: Delete `allowUnderpass` mapping (line 171).
- `compiler/transitionHelpers.ts`: Remove `allowUnderpass` passthrough (lines 126, 142).
- Theme presets in `themes/`: Delete `flowUnderpassDepth`, `flowUnderpassClearance`, `flowUnderpassPenalty` from all 6 preset files (`darkGlass`, `midnight`, `enterprise`, `neonCyber`, `lightMinimal`, `lightCanvas`).
- Theme presets in `packages/themes/src/presets/diagram/`: Same deletions in all 6 external theme files.
- Test fixtures: Remove `usedUnderpass: false` from all test data objects in `compiler/__tests__/flowPathBuilder.test.ts`, `compiler/__tests__/transitionHelpers.test.ts`, `compiler/__tests__/edgeGuidePlanner.test.ts`, `compiler/__tests__/edgeRoutingProfiles.test.ts`, `compiler/__tests__/edgePortPlanner.test.ts`, `__tests__/functionalTransitionSpec.test.ts`, `compiler/__tests__/edgeRenderOptimizer.test.ts`, `__tests__/diagramRenderer.test.ts`, `rendering/__tests__/EdgeRenderer.test.ts`, `compiler/__tests__/edgeCandidatePlanner.test.ts`, `compiler/__tests__/flowRouter.test.ts`, `compiler/__tests__/defaultsCompiler.test.ts`.

**Validation targets (all must pass):**
- `__tests__/archDiagramEdgeRouting.test.ts` (745 LOC) — full compilation with edge routing
- `player/__tests__/sceneCfOverviewRouting.test.tsx` — integration routing assertions
- `compiler/__tests__/transitionHelpers.test.ts` — blending + reroute path
- `__tests__/functionalTransitionSpec.test.ts` — transition spec (indirect: exercises `rerouteLiveEdges` through `functionalDiagramTransitionSpec`)
- All existing non-routing tests continue to pass (no changes to their files)

### Phase 9: Delete Old Pipeline

Once all tests pass with the new pipeline:

**Delete production code (16 files):**
...
- `compiler/edgeRouter.ts` (old one — replaced by `compiler/routing/edgeRouter.ts`)
- `compiler/edgeRenderOptimizer.ts` (old one — replaced by `compiler/routing/trunkOptimizer.ts`)

**Delete test code (12 files):**
- `compiler/__tests__/edgeRouter.test.ts`
- `compiler/__tests__/edgeCandidatePlanner.test.ts`
- `compiler/__tests__/edgePortPlanner.test.ts`
- `compiler/__tests__/edgeGuidePlanner.test.ts`
- `compiler/__tests__/edgeCandidateScorer.test.ts`
- `compiler/__tests__/edgeCandidateSelector.test.ts`
- `compiler/__tests__/edgeRoutingProfiles.test.ts`
- `compiler/__tests__/edgeRenderOptimizer.test.ts`
- `compiler/__tests__/flowRouter.test.ts`
- `compiler/__tests__/flowPathBuilder.test.ts`
- `compiler/__tests__/flowObstacleModel.test.ts`
- `compiler/__tests__/routingSpace.test.ts`
...
- `compiler/__tests__/transitionHelpers.test.ts` (updated in Phase 8)
- `compiler/__tests__/ghostNodeMerge.test.ts`
- `compiler/__tests__/hoverStateMachine.test.ts`
- `__tests__/archDiagramEdgeRouting.test.ts` (integration — keep, validates routing output)
- `__tests__/functionalTransitionSpec.test.ts` (transition — keep, validates reroute path)
- `player/__tests__/sceneCfOverviewRouting.test.tsx` (integration — keep)
...
```
Phase 1 (routingTypes.ts)
  ├── Phase 2 (sideSelect.ts)       ─┐
  │     └── Phase 3 (obstacleModel)   │  TRACK A
  │           └── Phase 4 (A* router) │
  │                                   │
  └── Phase 5 (pathBuilder.ts)       ─┘  TRACK B
        └── Phase 6a (shapeSnap)

Phase 6b (trunkOptimizer) — independent, can start after Phase 1

Phase 7 (orchestrator) — depends on ALL above (merge point)
Phase 8 (wire in + update callers) — depends on Phase 7
Phase 9 (delete old code) — depends on Phase 8
```

**Max parallelism: 2 developers.**
- **Developer A (Track A):** routingTypes → sideSelect → obstacleModel → orthogonalRouter
- **Developer B (Track B):** routingTypes → pathBuilder → shapeSnap + trunkOptimizer
- **Merge at Phase 7:** Either developer builds the orchestrator; both validate.
- **Phase 8–9:** Sequential, single developer.
...
|---|---|
| `routingTypes.ts` | Vec2 math edge cases: zero vectors, normalization of tiny vectors, clamp boundary values. |
| `sideSelect.ts` | Real `NodeRect` inputs → assert selected sides match XY geometry. Test locked ports, bundle hints, aspect-ratio weighting. Degenerate cases: co-located nodes (returns `'top'`), equal dx/dy magnitude (prefers vertical), single-edge port spreading. |
| `obstacleModel.ts` | Real node/group rects → assert obstacle set: includes correct nodes, excludes source/dest, respects container transparency, generates corridors for owning groups, soft vs hard classification. Degenerate cases: source and dest in same group (dual corridors), nested groups (independent evaluation). |
| `orthogonalRouter.ts` | Real obstacle sets → assert waypoints avoid obstacles, path is orthogonal, bend count is minimal. Test corridor punchthrough for owning groups. Degenerate cases: start === end (single waypoint), start inside obstacle (best-effort), zero obstacles (direct path). |
| `pathBuilder.ts` | `buildFlowPath`: arc rounding at 90° turns, no sharp corners. `buildCurvedPath`: single-waypoint straight shot vs multi-waypoint S-curve. `buildStraightPath`: fallback to flow when obstacles cause turns. `buildOrganicPath`: hash determinism (same inputs → same output). `assignDepth`: smoothstep Z at t=0, 0.5, 1 for same-Z and different-Z nodes. |
...
| `edgeRouter.ts` (orchestrator) | Unit: self-loop → empty route, missing endpoint → warn + empty route, profile dispatch. Integration: `archDiagramEdgeRouting.test.ts`, `sceneCfOverviewRouting.test.tsx`, `functionalTransitionSpec.test.ts` suites. |
...
|---|---|---|
| Production files | 16 | 8 (including orchestrator) |
| Production LOC | 5,067 | ~2,000 |
| Test files | 12 | 7 |
| Test LOC | 3,244 | ~1,500 (new, focused) |
| Total deleted | 28 files, 8,311 LOC | — |
