...
| **WS-4**: EngineARContainer alias | P3 | `EngineARContainer.tsx`, `player/index.ts` | None |

**Sequencing:** WS-1 and WS-4 have zero dependencies and can start immediately. WS-2 can start immediately (the prop names are known). WS-3 depends on WS-1 being merged first (it uses the new `theme` prop on `SceneReel`), but the developer can write all code before WS-1 lands — just needs WS-1 merged before the example compiles.
